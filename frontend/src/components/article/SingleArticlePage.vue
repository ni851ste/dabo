...
<script lang="ts">
    ...
    export default class SingleArticlePage extends Vue {
        ...
        get userName(): string {
            console.log("article is here in singleArticlePage", this.article)
            if (!this.user)
                return "";
        ...
        getArticleImages(): string[] {
            console.log("article in singlearticlePage " + this.article);
            if ((this.article.image === undefined) || (this.article.image.length < 1) || (this.article.image[0] === "")) {
                console.log("in here");
